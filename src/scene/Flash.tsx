...
void main() {
  ...
  // Nucleo duro con halo largo: el contraste entre los dos es lo que se lee
  // como energia en lugar de como una mancha luminosa.
  float core = pow(1.0 - smoothstep(0.0, 0.26, r), 3.0);
  float halo = pow(1.0 - smoothstep(0.0, 1.0, r), 2.2);
  ...
  float i = (core * 2.4 + halo * 0.85) * uFlash;
...
export function Flash() {
  ...
  useFrame((_, delta) => {
    ...
    if (mesh.current) {
      ...
      // barrido el encuadre.
      const s = 1 + sceneState.burst * 2.6;
